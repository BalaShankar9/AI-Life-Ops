...
import { TodayResponseSchema } from "@ai-life-ops/shared";
...
async function loadInitialSnapshot(): Promise<{
  ...
  try {
    ...
    const parsed = TodayResponseSchema.safeParse(json);
    if (!parsed.success) {
      return { snapshot: null, error: "Unable to load today's plan." };
    }

    return { snapshot: parsed.data.data.snapshot, error: null };
  } catch {
    return { snapshot: null, error: "Unable to reach the API." };
  }
}
